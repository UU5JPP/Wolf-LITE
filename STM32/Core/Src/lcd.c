...
//##################################################################################################################
	...
	LCDDriver_printTextFont(buff, LAY_STATUS_LABEL_PWM_X_OFFSET, LAY_STATUS_Y_OFFSET + LAY_STATUS_LABEL_PWM_Y_OFFSET, COLOR->STATUS_LABEL_VLT, BG_COLOR, LAY_STATUS_LABEL_STROKA_FONT);
		...
		//TRANSVERTER
	if ( TRX.Transverter_Enabled == true) {

		sprintf(ctmp, "TRANSVERTER");
		LCDDriver_printText(ctmp, LAY_STATUS_LABEL_TRS_X_OFFSET, LAY_STATUS_Y_OFFSET + LAY_STATUS_LABEL_TRS_Y_OFFSET, COLOR_RED, BG_COLOR, LAY_STATUS_LABELS_DBM_FONT_SIZE);
	}
